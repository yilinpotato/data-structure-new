...
本项目的 Q-learning 策略采用了**选择型超启发算法（Selection Hyper-heuristic）**架构，这是一种两层决策结构：

**低层启发式（Low-level Heuristics）**：
- `ExpertSelector.rank_candidates()` 使用组合启发式规则生成候选任务
- 综合考虑距离、重量、紧急度、电量安全性等多个因素
- 对所有可行任务进行评分排序，输出 Top-K（最多6个）候选任务

**高层启发式（High-level Heuristic）**：
- Q-learning 学习在候选任务中选择哪一个
- 不直接面对所有任务，而是在预筛选的候选集中决策
- 通过强化学习优化候选任务的选择策略

**超启发算法的优势**：
- 降低动作空间复杂度：从所有任务缩减到 Top-K 候选
- 结合专家知识与学习能力：低层使用领域启发式，高层学习选择策略
- 提高训练效率：Q-learning 只需学习在少量候选中选择，而非面对全部任务
- 保证决策质量：低层启发式过滤掉明显不可行或低质量的任务

这种架构使得 Q-learning 能够在有限的状态空间和动作空间内，学习到接近专家水平的调度策略。
